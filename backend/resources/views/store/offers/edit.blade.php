...
<div class="mb-6">
    <h1 class="text-2xl font-bold text-slate-800">تعديل العرض</h1>
    <p class="text-sm text-slate-500 mt-1">{{ $offer->title }}</p>
</div>

<form action="{{ route('store.offers.update', $offer->id) }}" method="POST" enctype="multipart/form-data" class="space-y-6">
    @csrf
    @method('PUT')
        ...
        <div class="lg:col-span-1 space-y-6">
            ...
            <div class="bg-white rounded-xl shadow-sm border border-slate-200 p-6">
                ...
                <div id="image-preview" class="w-full aspect-square rounded-xl bg-slate-100 border-2 border-dashed border-slate-300 flex items-center justify-center overflow-hidden mb-3">
                    @if($offer->image)
                        <img src="{{ Storage::url($offer->image) }}" class="w-full h-full object-cover" alt="{{ $offer->title }}">
                    @else
                ...
                <button type="button" id="image-btn" class="w-full bg-slate-100 hover:bg-slate-200 text-slate-700 px-4 py-2.5 rounded-lg font-semibold transition">تغيير الصورة</button>
                <p class="text-xs text-slate-400 mt-2 text-center">اتركه فارغاً للإبقاء على الصورة الحالية</p>
            </div>
            ...
            <div class="bg-gradient-to-br from-purple-50 to-indigo-50 rounded-xl shadow-sm border border-purple-200 p-6">
                ...
                <p class="text-xs text-purple-700 mb-4">أعد توليد الوصف باستخدام الذكاء الاصطناعي بناءً على الصورة الحالية.</p>
        ...
        <div class="lg:col-span-2 space-y-6">
            <div class="bg-white rounded-xl shadow-sm border border-slate-200 p-6 space-y-4">
                ...
                <div>
                    ...
                    <input type="text" name="title" id="title-input" value="{{ old('title', $offer->title) }}" class="w-full px-4 py-2.5 rounded-lg border border-slate-300 focus:ring-2 focus:ring-orange-500 focus:border-orange-500 outline-none" required>
                </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                        ...
                        <input type="number" name="original_price" id="original-price-input" value="{{ old('original_price', $offer->original_price) }}" step="0.01" min="0" class="w-full px-4 py-2.5 rounded-lg border border-slate-300 focus:ring-2 focus:ring-orange-500 focus:border-orange-500 outline-none" required>
                    </div>
                    <div>
                        <label class="block text-sm font-semibold text-slate-700 mb-1">سعر العرض (د.أ) <span class="text-rose-500">*</span></label>
                        <input type="number" name="offer_price" id="offer-price-input" value="{{ old('offer_price', $offer->offer_price) }}" step="0.01" min="0" class="w-full px-4 py-2.5 rounded-lg border border-slate-300 focus:ring-2 focus:ring-orange-500 focus:border-orange-500 outline-none" required>
                    </div>
                </div>
                ...
                <div>
                    ...
                    <textarea name="description" id="description-input" rows="5" class="w-full px-4 py-2.5 rounded-lg border border-slate-300 focus:ring-2 focus:ring-orange-500 focus:border-orange-500 outline-none" required>{{ old('description', $offer->description) }}</textarea>
                </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                        ...
                        <input type="datetime-local" name="start_date" value="{{ old('start_date', \Carbon\Carbon::parse($offer->start_date)->format('Y-m-d\TH:i')) }}" class="w-full px-4 py-2.5 rounded-lg border border-slate-300 focus:ring-2 focus:ring-orange-500 focus:border-orange-500 outline-none" required>
                    </div>
                    <div>
                        <label class="block text-sm font-semibold text-slate-700 mb-1">تاريخ الانتهاء <span class="text-rose-500">*</span></label>
                        <input type="datetime-local" name="end_date" value="{{ old('end_date', \Carbon\Carbon::parse($offer->end_date)->format('Y-m-d\TH:i')) }}" class="w-full px-4 py-2.5 rounded-lg border border-slate-300 focus:ring-2 focus:ring-orange-500 focus:border-orange-500 outline-none" required>
                    </div>
                </div>
                ...
                <div class="flex flex-wrap items-center gap-6">
                    <label class="flex items-center gap-2 cursor-pointer">
                        <input type="checkbox" name="is_active" value="1" class="w-4 h-4 text-emerald-600 rounded border-slate-300" {{ old('is_active', $offer->is_active) ? 'checked' : '' }}>
                        <span class="text-sm font-semibold text-slate-700">نشط</span>
                    </label>
                    <label class="flex items-center gap-2 cursor-pointer">
                        <input type="checkbox" name="is_featured" value="1" class="w-4 h-4 text-orange-600 rounded border-slate-300" {{ old('is_featured', $offer->is_featured) ? 'checked' : '' }}>
                        <span class="text-sm font-semibold text-slate-700">عرض مميز</span>
                    </label>
                    <label class="flex items-center gap-2 cursor-pointer">
                        <input type="checkbox" name="is_ai_generated" id="is-ai-generated" value="1" class="w-4 h-4 text-purple-600 rounded border-slate-300" {{ old('is_ai_generated', $offer->is_ai_generated) ? 'checked' : '' }}>
            ...
            <div class="flex items-center gap-3">
                <button type="submit" class="bg-orange-600 hover:bg-orange-700 text-white px-6 py-2.5 rounded-lg font-semibold transition">حفظ التغييرات</button>
...
const currentSubcategoryId = '{{ old("subcategory_id", $offer->subcategory_id) }}';
...
aiBtn.addEventListener('click', async () => {
    ...
    const offerPrice = document.getElementById('offer-price-input').value.trim();
    
    // ✅ جلب الصورة من الـ img tag
    const img = document.querySelector('#image-preview img');
    if (!img || !img.src || img.src.includes('data:image/svg')) {
        showAIError('لا توجد صورة للمنتج');
        return;
    }
    ...
    try {
        // ✅ جلب الصورة من الـ URL وتحويلها إلى File
        ...
        formData.append('image', imageFile);      // ✅ الصورة كملف
        formData.append('title', title);           // ✅ العنوان
        formData.append('offer_price', offerPrice); // ✅ السعر
        ...
        showAIError(err.message || 'حدث خطأ في الاتصال');
    } finally {
